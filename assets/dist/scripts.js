...
/*!
 * Generated using the Bootstrap Customizer (http://getbootstrap.com/customize/?id=bab18bb3545c8a1809f6349bbdfd15f8)
 * Config saved to config.json and https://gist.github.com/bab18bb3545c8a1809f6349bbdfd15f8
 */
...
jQuery(document).ready(function($) {
    ...
    // fonctions pour savoir si un element existe
    ...
    //lazyload
    $.lazyLoadXT.scrollContainer = '.gallery, .wrapper.news';
    $.extend($.lazyLoadXT, {
        edgeY: 300,
        edgeX: 300,
        srcAttr: 'data-src'
    ...
    if ($('.multi-gallery').exists()) {
        // flickity
        $('.multi-gallery .gallery').each(function() {
            var $gallerySlideshow = $(this).flickity({
                cellSelector: '.item',
                pageDots: false,
            ...
            $gallerySlideshow.on('click', function() {
                $gallerySlideshow.flickity('next');
            });
            var $galleryStatus = $(this).find('.gallery-status');
            var flkty = $gallerySlideshow.data('flickity');
            ...
            updateStatus();
            $gallerySlideshow.on('select.flickity', updateStatus);
        });
    }

    if ($('.gallery-double').exists()) {
        // flickity
        $('.gallery-double .gallery').each(function() {
            var $gallerySlideshow = $(this).flickity({
                cellSelector: '.item',
                pageDots: false,
                prevNextButtons: false,
            ...
            $gallerySlideshow.find('.element--image').on('click', function() {
                $gallerySlideshow.flickity('next');
            });
        });
    }

    $('.description__button').click(function(e) {
        $(this).text(function(i, text) {
            return text === "Text" ? "Close text" : "Text";
        })
        $(this).closest('.multi-gallery').toggleClass('with-description');
        return false;
    });

    // scripts lanc̩s lors d'un redimensionnement
    resize();
    ...
    // animation du scroll vers une ancre
    $(function() {
    ...
    $(".about-nav a").click(function() {
        $(".about-nav li").removeClass("current-menu-item");
        $(this).parent().addClass("current-menu-item");
    });


    if ($("body > div").hasClass("about")) {
        $('body').scrollspy({
            target: '#about-nav',
            offset: 100
        });
    ...
    // Force page reload with projects random anchors
    $('.nav li.current-menu-item a').click(function(e) {
        e.preventDefault();
        var linkHref = $(this).attr('href');
        if (linkHref.indexOf("#") != -1) {
            location.hash = linkHref.substr(linkHref.indexOf("#") + 1);
            location.reload();
...
function resize() {
    if ("matchMedia" in window) { // D̩tection
        if (window.matchMedia("(min-width:992px)").matches) {
            // d̩placement de la description un cran plus haut
            $('.description__header').each(function() {
                var wrapper = $(this).closest('.gallery-wrapper').find('.row');
...
function scroll(dir) {
    item_increment = (dir >= 1) ? 1 : 0;
    current_position = $(window).scrollTop();
    inner_height = parseFloat($('.gallery-wrapper').first().outerHeight(true));
    total_height = $(document).height();
    total_items = Math.floor(total_height / inner_height);
    current_item = Math.round(current_position / inner_height);
    current_item = ((current_item + item_increment) == total_items ? -1 : current_item);
    // Loop to bottom if first element previous
    if (current_item == 0 && item_increment == 0) {
        scroll_to_position = inner_height * total_items;
    } else {
        scroll_to_position = inner_height * (current_item + dir);
    }
    $('html, body').animate({
